  ...
  function extractMediaUrls(item) {
    ...
    if (isVideo) {
      if (Array.isArray(item.video_versions) && item.video_versions.length > 0) {
        // Sort by width/height to get highest resolution
        const sortedVideos = [...item.video_versions].sort((a, b) => (b.width * b.height) - (a.width * a.height));
    ...
    // 2. Image / Display URLs
    if (Array.isArray(safeGet(item, 'image_versions2.candidates'))) {
      const candidates = item.image_versions2.candidates;
      const sortedCandidates = [...candidates].sort((a, b) => (b.width * b.height) - (a.width * a.height));
      ...
      const sortedRes = [...item.display_resources].sort((a, b) => (b.config_width * b.config_height) - (a.config_width * a.config_height));
